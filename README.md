...
| `/activity on\|off` | Enable or disable thinking/tool activity segments; off restores the existing `TOOL_VERBOSITY` presentation; bare `/activity` reports the current state |
...
After session activation has finished extension binding and callback subscription, TelePi sends a temporary `⏳ Working…` message with `⏹ Abort` immediately before prompting. The first visible activity, assistant, or legacy tool output replaces that message in place, so normal output does not gain a separate status row. When `ask_user` opens its Telegram-native dialog, TelePi serializes control handoff with pending output: it removes an unused temporary status or neutralizes Abort on already adopted content before sending the dialog, so Cancel is the only persistent interaction control. A silent success removes the temporary message, while an ordinary early abort or failure turns it into the terminal status. After a successful dialog handoff, a failure sends one standalone notice; if status deletion failed, the neutralized surviving Working message remains the failure target. The Abort button then follows newer output messages until the run settles; `/abort` remains available as a fallback. Typing starts immediately, refreshes every 4.5 seconds, and remains active until the prompt succeeds, fails, is aborted, or cannot activate. Output changes kind by sealing the current segment and appending the next activity or assistant segment, while adjacent events of the same kind continue in the open message (with Telegram-sized continuation chunks when needed). Thinking is rendered with the compact plain `🧠 Thinking` heading, and a tool row follows it after one empty line. Ordinary follow-on text during the same active run steers Pi; media and commands are not steering inputs.
...
│   │   ├── prompt-handler.ts     ← chronological prompt delivery, Abort ownership, and typing lifecycle
│   │   ├── stream-segments.ts     ← ordered activity/assistant segments and delivery metadata
